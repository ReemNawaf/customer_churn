...
def parse_raw_events() -> pd.DataFrame:
    """
    ...
    log.info(f"Loading raw data from {obj}")
    ...
    parsed = ua_series.fillna("").apply(_parse_user_agent)
    if not parsed.empty:
    ...
    normalized_loc = loc_series.fillna("").apply(_normalize_location)
...
def _to_dataframe(obj: Jsonish) -> pd.DataFrame:
...
def _parse_user_agent(ua: str) -> tuple[str | None, str | None, str | None]:
    ...
    os_name = _match_first(ua, _OS_PATTERNS)
    browser = _match_first(ua, _BROWSER_PATTERNS)
...
def _normalize_location(loc: str) -> tuple[str | None, str | None, str | None]:
    """
