...
def build_all_features(df_logs: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
    ...
    demo = build_demographics_features(df_feat, cutoff_date)
    engage = build_engagement_features(df_feat, cutoff_date)
    consist = build_consistency_features(df_feat, cutoff_date)
    sessions = build_session_features(df_feat, cutoff_date)
    diversity = build_diversity_features(df_feat, cutoff_date)
    account = build_account_features(df_feat, cutoff_date)
    derived = build_derived_features(demo, engage, consist, sessions, diversity, account, cutoff_date)
...
# 1. Cut-Off Date
# -------------------------------------------------------------------
def compute_cutoff_date(df: pd.DataFrame, date_col: str = "ts_dt") -> pd.Timestamp:
    """
...
# 1. Demographics
# -------------------------------------------------------------------
def build_demographics_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
# 2. Engagement
# -------------------------------------------------------------------
def build_engagement_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
# 3. Consistency & Trend
# -------------------------------------------------------------------
def build_consistency_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
# 4. Session-Level
# -------------------------------------------------------------------
def build_session_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
# 5. Diversity
# -------------------------------------------------------------------
def build_diversity_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
# 6. Account / Level
# -------------------------------------------------------------------
def build_account_features(df: pd.DataFrame, cutoff_date: pd.Timestamp) -> pd.DataFrame:
    """
...
def build_derived_features(
    demo: pd.DataFrame, engage: pd.DataFrame, consist: pd.DataFrame, sessions: pd.DataFrame, diversity: pd.DataFrame, account: pd.DataFrame, cutoff_date: pd.Timestamp
) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    raw_path = obj = f"{Path().resolve()}/{cfg['data']['processed_path']}"
    out_path = obj = f"{Path().resolve()}/{cfg['data']['features_path']}"

    df_logs = pd.read_parquet(raw_path)
    cutoff_date = compute_cutoff_date(df_logs)

    features_df = build_all_features(df_logs, cutoff_date)
